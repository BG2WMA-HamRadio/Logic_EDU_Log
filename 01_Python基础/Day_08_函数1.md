...
#### 不定长参数
  ...
  ```
  - `*形参`只能有一个，如果`*形参`不在最后边的时候，则传递实参的时候，`*形参`后边的所有实参都要使用关键字传参。
    ```
    def fnx(a, b, *c):
        print('a = ', a)
        print('b = ', b)
        print('c = ', c)
    fnx(1, 2, 3, 4, 5)          # a = 1, b = 2, c = (3, 4, 5)
    ```
  - `*形参`只能接收位置传参，不能接收关键字传参
  - `**形参`可以接收关键字传参，它会将这些参数统一保存到字典当中。字典的key是参数的名字，字典的value就是传递的参数值。
  - `**形参`只能由一个，且必须放在所有形参的最后边。
  ```
  def fn3(**b):
      print('b = ', b)
      
  fn3(b= 1, d= 2, c= 3)      # b =  {'b': 1, 'd': 2, 'c': 3}
  # 将传递的参数以字典的形式保存
  ```
  - 不定长参数的标准写法：  
  ```
  def fn(*args, **kwargs):
      语法块
  ````
  `*args`就是位置参数，`**kwargs`就是关键字参数。
  
  - 参数的解包
    - 传递实参时，也可以在序列类型的参数前边添加`*`，这样它会自动地将序列中地元素依次作为参数传递。
    ```
    def fnx(a, b, c):
        print('a = ', a)
        print('b = ', b)
        print('c = ', c)
    t = (1, 2, 3)
    fnx(t[0], t[1], t[2])             # 取出元组中的每一个数值传递给相应的形参
    fnx(*t)                           # 自动将元组中的每一个数值传递给形参，元素的数量必须和形参的数量一致。
    ```  
    - 要求序列中元素个数必须跟形参的个数一致      
    ```  
    def fnx(name, callsign, grid):
        print('name = ', name.title())
        print('callsign = ', callsign.upper())
        print('grid = ', grid)
    d = {'name': 'wang tiangang', 'callsign': 'bg2wma', 'grid': 'PN11qu'}
    fnx(**d)                          # 将字典中的元素按关键字传递
    ```
